...
document.getElementById("user-info").innerHTML =   <div class="user-card">   <h3>${escapeHtml(user.name || "")}</h3>   <p>الفرقة: ${escapeHtml(user.grade || "")}</p>   <p>الفصل: ${escapeHtml(user.section || "")}</p>   <p>الحالة الصحية: ${escapeHtml(user.health || "")}</p>   <p>التقييم: ${escapeHtml(user.rating != null ? user.rating : "")}</p>   ${user.absent ?<p>أيام الغياب: ${escapeHtml(user.absent)}</p>: ""}   <h4>الإنجازات:</h4>   <ul>${achievementsHTML}</ul>   </div>  ;
hideAll();
...
case "internet-research":  
  html = `<h3>مسابقة بحثية عن أخطار الإنترنت</h3>  
          <p>الموضوع: "الإنترنت سلاح ذو حدين: كيف تستفيد من مميزاته وتقاوم سلبياته"</p>  
          <p>التسجيل: الطلاب الراغبون يقدّمون أسماءهم إلى الطالب <strong>مصطفى أشرف</strong> (المقرر الديني والثقافي)، فصل 2/10</p>`;  
  break;  

case "big-religion-students":  
  html = `<h3>المسابقة الدينية الكبرى — للطلاب</h3>  
          <p>الموعد النهائي: <strong>26 / 3 / 2026</strong></p>  
          <p>تسليم الأبحاث إلى الطالب <strong>مصطفى أشرف</strong> المقرر الديني والثقافي لاتحاد الطلاب.</p>`;  
  break;  

case "big-religion-teachers":  
  html = `<h3>المسابقة الدينية الكبرى — للمعلمين</h3>  
          <p>الموعد النهائي: <strong>26 / 3 / 2026</strong></p>  
          <p>إرسال أسماء المشتركين إلى الأستاذ <strong>أميل</strong>.</p>`;  
  break;  

case "big-religion-parents":  
  html = `<h3>المسابقة الدينية الكبرى — لأولياء الأمور</h3>  
          <p>الموعد النهائي: <strong>26 / 3 / 2026</strong></p>  
          <p>إرسال أسماء المشتركين إلى الأستاذ <strong>أميل</strong>.</p>`;  
  break;  

case "short-story":  
  html = `<h3>مسابقة كتابة القصة القصيرة</h3>  
          <p>المطلوب: كتابة قصة قصيرة عن أحداث حرب أكتوبر بطريقة شيّقة.</p>  
          <p>الموعد النهائي للتسليم: <strong>1 / 10 / 2025</strong></p>`;  
  break;  

case "music-competition":  
  html = `<h3>مسابقة الموسيقى</h3>  
          <p>المطلوب: إعداد أغنية وطنية عن حرب أكتوبر.</p>  
          <p>الموعد النهائي: <strong>1 / 10 / 2025</strong></p>  
          <p>التقديم: التوجه إلى غرفة الموسيقى للتسجيل.</p>`;  
  break;  
